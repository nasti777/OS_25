//Двунаправленная связь через pipe между родительским и дочерним процессом
#include<unistd.h>
#include<stdlib.h>
#include<stdio.h>
#include<sys/types.h>

int main() {
	int fd1[2], fd2[2], result;
	//fd1 - файловые дескрипоторы для передачи информации от родителя к ребёнку
	//fd2 - файловые дескрипторы для передачи информации от ребёнка к родителю
	size_t size;
	char resstring1[86], resstring2[92];
	//resstring1 - строка, полученаая процессом-ребёнка
	//resstring2 - строка, полученная процессом-родителем
	if((pipe(fd1) <0)||(pipe(fd2)<0)) {
		printf("Не удалось создать пайп\n");
		exit(-1);
	}
	result = fork();
	if(result < 0) {
		printf("Ошибка при выполнении fork()\n");
		exit(-1);
	}
	else if (result > 0) {
		if ((close(fd1[0]) < 0)||(close(fd2[1]) < 0)) {
			printf("Не удалось закрыть входной или выходной поток процесса-родителя\n");
			exit(-1);
		}
		size = write(fd1[1], "Информация для дочернего процесса от родителя", 86);
		if(size != 86) {
			printf("Родителю не удалось записать всю строку в пайп\n");
			exit(-1);
		}
		if (close(fd1[1]) < 0) {
			printf("Родителю не удалось закрыть выходной поток\n");
			exit(-1);
		}
		printf("Читаем строку, отправленную процессом-ребёнком ...\n");
		size = read(fd2[0], resstring2, 92);
		if(size < 0) {
			printf("Процессу-родителю не удалось прочитать строку\n");
			exit(-1);
		}
		printf("Прочитанаая родителем строка: %s\n", resstring2);
		if (close(fd2[0]) < 0) {
			printf("Не удалось закрыть входной поток процесса-ребёнка\n");
			exit(-1);
		}
		printf("Процесс родитель завершил работу\n");
	}
	else {
		if((close(fd1[1])<0)||(close(fd2[0])<0)) {
			printf("Не удалось закрыть выходной или входной поток процесса-ребёнка\n ");
			exit(-1);
		} 
		size = read(fd1[0], resstring1, 86);
		if(size < 0) {
			printf("Процессу-ребёнку не удалось прочитать строку\n");
			exit(-1);
		}
		printf("Строка, прочитанная дочерним процессом: %s\n", resstring1);
		if (close(fd1[0])<0) {
			printf("Не удалось закрыть входной поток процесса-ребёнк\n");
			exit(-1);
		}
		size = write(fd2[1], "Информация для родительского процесса от ребёнка", 92);
		if(size != 92) {
			printf("Ребёнку не удалось записать всю строку в пайп\n");
			exit(-1);						                
		}
		if (close(fd2[1])<0) {
			printf("Не удалось закрыть выходной поток процесса-ребёнка\n");
			exit(-1);
		}
		printf("Процесс-ребёнок завершил работу\n");
	}
	return 0;
}
